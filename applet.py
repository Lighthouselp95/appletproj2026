...
def main():
    ...
    screen_in_item.connect("activate", run_command,  'sudo ddcutil setvcp 10 + 5 ; echo "/n Increase +5"')

    menu.append(screen_in_item)
    # I'm add a screen decrease object
    screen_de_item = Gtk.MenuItem(label="decrease brightness")
    screen_de_item.connect('activate', run_command, 'sudo ddcutil setvcp 10 - 5 ; echo "/n Decrease -5"')
